   ...
   namespace detail {
      ...
      Real finite_difference_derivative(const F f, Real x, Real* error, const fd_tag<8>&)
      {
         ...
         if (error)
         {
            ...
            *error = abs(f9) / (630 * h) + 7 * (abs(yh) + abs(ymh))*eps / h;XX9236
         }
         return (tmp1 + tmp2) / (105 * h);
      }

      template<class F, class Real>
      Real vec_finite_difference_derivative(const F f, Real* x, int pos, int input_size, int output_size, Real* error, const fd_tag<8>&)
      {
         using std::sqrt;
         using std::pow;
         using std::abs;
         ...
         auto x_h = new Real[input_size];
         auto ymh = new Real[output_size];
         auto y1 = new Real[output_size];
         auto y2 = new Real[output_size];
         auto y3 = new Real[output_size];
         auto y4 = new Real[output_size];
...
//         y1 = yh - ymh;
      ...
      template<class F, class Real, class tag>
      Real vec_finite_difference_derivative(const F, Real, int *, int, Real*, const tag&)
      {
         // Always fails, but condition is template-arg-dependent so only evaluated if we get instantiated.
         static_assert(sizeof(Real) == 0, "Finite difference not implemented for this order: try 1, 2, 4, 6 or 8");
      }
   ...
   template<class F, class Real, size_t order=6>
   inline Real vec_finite_difference_derivative(const F f, Real* x, int* stencil, int input_size, int output_size, Real* error = nullptr)
   {
      return detail::vec_finite_difference_derivative(f, x, stencil, input_size, output_size, error, detail::fd_tag<order>());
   }
}  // namespaces
#endif
