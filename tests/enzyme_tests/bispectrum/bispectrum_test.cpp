...
int main(){
    ...
    double grad[64][3];
    double dzeta[64 * 55];
    double kliffzeta[64][55];
    int * neighbors[64];
    int n_neigh[64];
    int * species;
    auto desc = new double[55 * 64];
    int image[259];
    ...
    auto dbs = Descriptor::DescriptorKind::initDescriptor(file_name,Descriptor::KindBispectrum);
    ...
    for(int i = 0; i < 64; i++) {
        Descriptor::compute_single_atom(i,
                                        ...
                                        dbs);
        for (int j = 0; j < 55; j++){
            aggregated_error += abs(kliffzeta[i][j] - *(desc + i * 55 + j));
        }
    }
